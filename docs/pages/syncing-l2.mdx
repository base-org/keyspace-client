...
:::note
Since the L1 block hash changes frequently, it can be difficult to generate a proof and submit it onchain before the L1 block hash changes. Instead, syncing can be implemented as a two step process where the current block hash is read from the oracle and stored in the L2 KeyStore contract. That gives you a fixed block hash to generate proofs from where the expiry time is under your control instead of the L2's rollup protocol.
:::
